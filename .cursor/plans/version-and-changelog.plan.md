...
| **MINOR** | Каждая **сборка Docker-образа и успешный push на Docker Hub** | Скрипт в конвейере push ([scripts/docker-hub-build-push.sh](scripts/docker-hub-build-push.sh), и при появлении — GitHub Actions): перед `docker build` увеличить MINOR на 1 и записать в `VERSION`. Локальный `docker compose build` без push **не** обязан трогать MINOR. |
